...
public class ActionChamberingOut extends Action {
	...
	public boolean onActionBegin(World world, EntityPlayer player, InfWeapon inf) {
		...
		if(chambering == 0 ||curItem == null || !(curItem.getItem() instanceof WeaponGenericE))
			return false;
		if(chambering != 1){
	...
	public boolean onActionEnd(World world, EntityPlayer player, InfWeapon inf) {
		ItemStack curItem = player.getCurrentEquippedItem();
		WeaponGenericE wpnType = (WeaponGenericE) curItem.getItem();
		if(WeaponHelperE.getChamber(curItem) != 0 || curItem == null || !(curItem.getItem() instanceof WeaponGenericE || inf.getTickLeft(this)>0))
			return false;
		//若不是全手动，会恢复上膛
