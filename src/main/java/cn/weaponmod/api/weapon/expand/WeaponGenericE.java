...
import cn.weaponmod.api.action.expand.ActionAutomaticShootE;
import cn.weaponmod.api.action.expand.ActionChamberingIn;
import cn.weaponmod.api.action.expand.ActionChamberingOut;
import cn.weaponmod.api.action.expand.ActionReloadEBulletIn;
import cn.weaponmod.api.action.expand.ActionReloadEBulletOut;
import cn.weaponmod.api.action.expand.ActionReloadEClipIn;
 ...
 * @author Nolife_M
 */
public class WeaponGenericE extends WeaponGeneric{
	...
	public WeaponGenericE(String name,Item ammo ,boolean consumeInv) {
		super(ammo);
	...
	//设置List内加入支持的弹夹
	public WeaponGenericE addAmmo(Item item){
		this.List.add(item);
		return this;
	}
	...
	//设置可自动上弹
	public WeaponGenericE setAutoLoading(boolean b){
		this.autoLoading = b;
		return this;
	}
	...
	//可空仓挂机
	public WeaponGenericE setBoltCatch(boolean b){
		this.boltCatch = b;
		return this;
	}
	...
	//全手动
	public WeaponGenericE setFullManual(boolean b){
		this.fullManual = b;
		return this;
	}
	...
	//直接装膛
		public WeaponGenericE setDirectLoading(boolean b){
			this.directLoad = b;
			return this;
		}
		...
		//快速清仓
		public WeaponGenericE setFastClear(boolean b){
			this.fastClear = b;
			return this;
		}
	...
	public void onItemClick(World world, EntityPlayer player, ItemStack stack, int keyid) {
		...
		case 2: //Reload
			...
			&& WeaponHelperE.getAmmo(stack) < stack.getMaxDamage()){//执行装填部分
				...
				if(WeaponHelperE.tryGetAmmo(((WeaponGenericE)stack.getItem()).getAmmoList(), player) != null){
					Item ammo = WeaponHelperE.tryGetAmmo(((WeaponGenericE)stack.getItem()).getAmmoList(), player).getItem();
					if( ammo != null){
						//判断ammo类型
